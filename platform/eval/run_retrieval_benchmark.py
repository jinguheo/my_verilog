...
def resolve_version(ver_dir: str | None, version_tag: str | None, approved_labels: str | None):
    """Return (approved_labels_path, version_tag) resolved from version snapshot if requested."""
    if not version_tag:
    ...
    snapped_labels = snap / "auto_approved_labels.jsonl"
    resolved = str(snapped_labels) if snapped_labels.exists() else approved_labels
    return resolved, version_tag
...
def main():
    ...
    # Version-specific output sub-directory
    ...
    retrieval_metadata["kg_version_tag"] = version_tag
