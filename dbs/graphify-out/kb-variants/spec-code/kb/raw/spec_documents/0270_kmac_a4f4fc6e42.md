# Spec Document: opentitan/hw/ip/kmac/data/kmac.hjson

- Project: `opentitan`
- Original source: `D:\MyWork\verilog\dbs\opentitan\hw\ip\kmac\data\kmac.hjson`
- Exported path: `D:\MyWork\verilog\out\spec_documents_20260514_204108\opentitan\hw\ip\kmac\data\kmac.hjson`
- Original extension: `.hjson`
- Original bytes: 41807
...

{
  ...
  features: [
    {
      name: "KMAC.MODE.SHA3",
      desc: '''SHA3 hashing algorithm supporting the following security strength configurations:
            SHA3-224, SHA3-256, SHA3-384, and SHA3-512. Compliant with NIST FIPS 202.
            '''
    }
    {
      name: "KMAC.MODE.SHAKE",
      desc: '''SHAKE hashing algorithm supporting the following security strength configurations:
            SHAKE128, and SHAKE256. Compliant with NIST FIPS 202.
            '''
    }
    {
      name: "KMAC.MODE.CSHAKE",
      desc: '''cSHAKE hashing algorithm supporting the following security strength configurations:
            cSHAKE128, and cSHAKE256. Compliant with NIST SP 800-185.
            '''
    }
    {
      name: "KMAC.MODE.KMAC",
      desc: '''KMAC Keccak Message Authentication Code supporting the following security strength
            configurations: KMAC128, KMAC256. Compliant with NIST SP 800-185.
            '''
    }
    {
      name: "KMAC.MODE.XOF",
      desc: '''SHAKE, cSHAKE and KMAC modes of operation support eXtendable-Output Format (XOF)
            as specified in NIST SP 800-185.
            '''
    }
    {
      name: "KMAC.ENDIANNESS.MESSAGE",
      desc: "Configurable input message endianness."
    }
    {
      name: "KMAC.ENDIANNESS.DIGEST",
      desc: "Configurable output digest endianness."
    }
    {
      name: "KMAC.KEY.SIDELOAD",
      desc: "Key can be loaded directly from the keymgr."
    }
    {
      name: "KMAC.ENTROPY_MODES",
      desc: '''Entropy required for countermeasures can be configured to be provided by software or
            EDN. In EDN mode, KMAC can be configured to set an error flag if entropy is not
            provided within a configurable time window. The cadence of entropy requested from EDN
        ...
        { bits: "17:16"
          ...
          enum: [
            ...
            { value: "1"
              ...
              The module will then automatically reseed the internal PRNG with fresh entropy from EDN every !!ENTROPY_REFRESH_THRESHOLD_SHADOWED messages in KMAC mode.
              More precisely, automatic reseed operations are always triggered directly after the key absorption phase.
              How close the module is to automatically trigger a reseed operation can be inferred from !!ENTROPY_REFRESH_HASH_CNT.
              Alternatively, software can manually initiate reseeding operations using !!CMD.entropy_req while the module is idle.
              This will also clear !!ENTROPY_REFRESH_HASH_CNT.

              Depending on the state of the system, it may take a very long or even indefinite time for the entropy to be delivered.
              As such, the configuration values for the !!ENTROPY_REFRESH_THRESHOLD_SHADOWED and !!ENTROPY_PERIOD registers should be chosen carefully.
              Also, if the entropy complex becomes unavailable, there is a way to leave this mode and enter the fall-back software mode.
              For guidance, refer to [EDN Entropy Mode](programmers_guide.md#edn-entropy-mode).
              '''
            }
            { value: "2"
              name: "sw_mode"
              ...
              Once the !!CFG_SHADOWED.entropy_ready bit is set after reset, the module will wait for software to write the !!ENTROPY_SEED register 6 times.
              Upon each write, the written value is loaded into the corresponding state chunk of the entropy generator.
              Only after that, the module can start processing commands.
              Depending on !!CFG_SHADOWED, the internal PRNG is then used for (re-)masking inputs (prefix, key, message) and intermediate results of the Keccak core.

              After this point, the PRNG can no longer be reseeded by software.
              Without a full reset of the KMAC block, there is no way to further change the configured mode.
              Further writes to this field only change the value received upon reading the register.
              The changes are ignored by the internal hardware.
              '''
            }
          ]
