# Spec Document: opentitan/hw/ip/lc_ctrl/data/lc_ctrl_sec_cm_testplan.hjson

- Project: `opentitan`
- Original source: `D:\MyWork\verilog\dbs\opentitan\hw\ip\lc_ctrl\data\lc_ctrl_sec_cm_testplan.hjson`
- Exported path: `D:\MyWork\verilog\out\spec_documents_20260514_204108\opentitan\hw\ip\lc_ctrl\data\lc_ctrl_sec_cm_testplan.hjson`
...
// .../lc_ctrl/data/lc_ctrl_testplan.hjson
{
  testpoints: [
    ...
    }
    {
      ...
      2) `lc_nvm_rma_ack_i`: Verify that an inconsistent input value
      (`lc_nvm_rma_ack_i` != `lc_nvm_rma_ack_o`) causes a NVM_RMA_ERROR
      when a life cycle transition is initiated.

      There are two ways this condition can be triggered: either no RMA
      transition is requested, and then setting `lc_nvm_rma_ack_i` to any
      ...
      `lc_nvm_rma_ack_i` to any value other than 'ON' after the main FSM
      has progressed through the `NvmRmaSt` state should trigger this
      error.
