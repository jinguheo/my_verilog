# Spec Document: opentitan/hw/top_darjeeling/data/chip_testplan.hjson

- Project: `opentitan`
- Original source: `D:\MyWork\verilog\dbs\opentitan\hw\top_darjeeling\data\chip_testplan.hjson`
- Exported path: `D:\MyWork\verilog\out\spec_documents_20260514_204108\opentitan\hw\top_darjeeling\data\chip_testplan.hjson`
- Original extension: `.hjson`
- Original bytes: 137826
...
// SPDX-License-Identifier: Apache-2.0
{
  ...
  testpoints: [
    /////////////////////////////////////////////////////////////
    // IO Peripherals                                          //
    // UART, GPIO, I2C, SPIDEV, SPIHOST, PINMUX & PADCTRL, MBX //
    /////////////////////////////////////////////////////////////
    ...
    }
    {
      ...
      desc: '''Verify the pass through mode from an end-to-end perspective.
            ...
            - Configure and enable spi_host0
    ...
    }
    {
      ...
      desc: '''Verify the MIO output values in any sleep states.
            ...
            In this test, passthrough feature is not tested.
            '''
      stage: V2
    ...
    }
    {
      ...
      desc: '''Verify the retention logic in pinmux that is activated during deep sleep.
            ...
            - Pick a pin (such as GPIO0) and enable it in output mode. Set a known value to it (0 or
              1) and verify the correctness of the value on the chip IO.
    ...
    {
      name: chip_sw_mbx_smoke
      desc: '''Verify the mailbox integrations with a basic smoketest

            - Random-cyclically select a mailbox to perform a req-rsp transaction with
            - TB(requester) writes a message into the imbx, then awaits a response
            - sw(responder) awaits the request, reads it then replies with a modified version
              of the original message
            - TB checks if the received response is as-expected
            - Requester randomly selects between an irq or polling-based handler
            - Responder polls the peripheral to await transaction completed
            - Repeat process for all mailboxes in the system
            '''
      stage: V2
      tests: ["chip_sw_mbx_smoke"]
    }
    ...
    }
    {
      ...
      desc: '''Verify non-debug reset request initiated from RV_DM when the chip is awake.
            ...
            - Configure RV_DM to send NDM reset request to reset system reset tree.
    ...
    }
    {
      ...
      desc: '''Verify that the chip can go into deep sleep state and be reset up by ALL reset req
            ...
            - POR (HW PAD) reset, SW POR, wdog timer reset, esc rst, SW req
    ...
    // RSTMGR tests:
    {
      ...
      desc: '''Verify the `reset_info` CSR register for lc or higher resets.
            ...
            blocks are `pwrmgr`, `alert_handler` and `aon_timer`.
    ...
    }
    {
      ...
      desc: '''Verify alert_handler's ping mechanism works correctly during sleep and wake up.
            ...
            - Configure alert_handler's ping timeout register to a reasonable value that won't cause
   ...
   }
   {
      ...
      desc: '''Verify escalation reverse ping timer disabled in sleep mode.
            ...
            The alert escalation responder inside `pwrmgr` is connected to the `io` clock,
    ...
    }
    {
      ...
      desc: '''Verify the keymgr div output to keymgr.
            ...
            '''
      stage: V2
      tests: ["chip_sw_keymgr_dpe_key_derivation_prod"]
    }
    {
      ...
      desc: '''Verify broadcast signals from lc_ctrl.
            ...
            - Verify that the following broadcast signals are having the right effect in the
              ...
              - lc_creator_seed_sw_rw_en_o: impacts otp_ctrl
              - lc_seed_hw_rd_en_o: impacts otp_ctrl
    ...
    {
      name: chip_sw_aes_gcm
      desc: '''Verify the AES-GCM operation.

            - Put the AES hardware IP block into the initialization phase of GCM.
            - Write the initial key shares and the IV into CSRs (known combinations).
            - Put the AES hardware IP block into the additional authenticated data (AAD) phase of
              GCM.
            - Write the first known AAD block into the input data CSRs.
            - Write the second known AAD block into the input data CSRs.
            - Put the AES hardware IP block into the encryption/decryption phase of GCM.
            - Write the first known plaintext block into the input data CSRs.
            - Retrieve the first ciphertext block from the output data CSRs.
              - Repeat this procedure for three additional blocks.
            - Check the retrieved ciphertext blocks against the expected ciphertext values.
            - Put the AES hardware IP block into the tag phase of GCM.
            - Write the AAD and plaintext lengths into the input data CSRs.
            - Retrieve the generated authentication tag from the output data CSRs and compare it
              against the expected tag.
            - Repeat this whole procedure 3 more times but interrupt the processing at different
              points in time by saving GHASH state, clearing state of the AES hardware IP block,
              restoring the GHASH state and then resuming the processing.
              - During the first repetition, the GHASH state is saved after the first AAD block.
              - During the second repetition, the GASH state is saved after the first plaintext
                block.
              - During the last repetition, the GHASH state is saved after the first AAD block
                and then again after the first plaintext block.
            '''
      stage: V2
      tests: ["chip_sw_aes_enc",
              "chip_sw_aes_enc_jitter_en"]
    ...
    }
    {
      ...
      desc: '''Verify the keymgr interface to KMAC.
            ...
            '''
      stage: V2
      tests: ["chip_sw_keymgr_dpe_key_derivation"]
    }
    {
    ...
    // ENTROPY_SRC (pre-verified IP) integration tests:
    {
      ...
      desc: '''Verify the transfer of entropy bits to CSRNG.
            ...
            - Disable edn0, edn1, and csrng, as these are enabled by the test ROM.
    ...
    }
    {
      ...
      desc: '''Verify the fuse input to CSRNG.
            ...
            - Verify that SW can read the internal state values.
             ...
             Note that this test has been simplified to just test the test_fuse_enable path,
             since the HW has been updated to always hardwire the EN_CSRNG_SW_APP_READ
             chicken switch to True. The chicken switch itself has been removed from OTP.
            '''
      stage: V2
    ...
    // KEYMGR_DPE (pre-verified IP) integration tests:
    {
      ...
      desc: '''Verify the keymgr advances to all states and generate identity / SW output.
            ...
            '''
      stage: V2
      tests: ["chip_sw_keymgr_dpe_key_derivation", "chip_sw_keymgr_dpe_key_derivation_jitter_en"]
    }
    {
    ...
    }
    {
      ...
      desc: '''Verify OTBN can fetch RND numbers from the entropy src.
            ...
            - The OTBN app ensures that the values when read consecutively do not match, and it's
              not all 0s or all 1s, as a basic measure to ensure that the entropy subsystem is
              returning some data.
            '''
      stage: V2
    ...
    }
    {
      ...
      desc: '''Verify the otp program error.
            ...
            - Keep state across resets in flash to check that no NMI or regular ISR interrupt run
              for kTopDarjeelingAlertIdLcCtrlFatalProgError.
    ...
    }
    {
      ...
      desc: '''Verify the NVM strike counter partition in otp_ctrl.

            - Check that the partition is all-zeroes first.
            - Increment the strike counter by 1 multiple times around different count offsets, e.g.:
              - start with offset zero, increment multiple times
              - set counter to 30 (which is just before the first word boundary) and increment a few times
              - set counter to a higher count close to a word boundary and increment a few times
            - Test error cases by
              - injecting errors that can be recovered by OR'ing the two counter copies.
              - injecting errors that cannot be recovered by OR'ing the two counter copies.
            - Set counter to a value close to the maximum and let it saturate. This will overwrite
              previously corrupt encodings with all-ones, thus resulting in a consistent encoding.
            - Reset the chip and check that the counter is still maximal.
            '''
      stage: V2
      tests: ["chip_sw_otp_ctrl_nvm_cnt"]
    }
    {
      name: chip_sw_otp_ctrl_sw_parts
      desc: '''Verify that SW partitions can be programmed and read/write locked.

            This test operates on three representative SW partitions with
            different attributes and we rely on block-level coverage for the
            remaining partitions:
            - OWNER_SW_CFG (read and write-lockable)
            - OWNERSHIP_SLOT_STATE (only read-lockable)
            - EXT_NVM (only read-lockable, no ECC)

            The test checks:
            - That the first word of each partition is writable.
            - That the dynamic read locks work for each partition.
            - That the permanent write lock works for OWNER_SW_CFG only.
            '''
      stage: V2
      tests: ["chip_sw_otp_ctrl_sw_parts"]
    }
    ...
    // soc_proxy tests:
    {
      name: chip_sw_soc_proxy_external_reset_requests
      desc: '''Verify that external reset requests reset (parts of) OT.

               - In DV env, trigger the external reset request.
               - In DV env, ensure that the desired parts of OT get reset and the others don't.
               - In SW, ensure that the reset request shows up in Reset Manager's RESET_INFO register.
           '''
      stage: V2
      tests: ["chip_sw_soc_proxy_smoketest"]
    }
    {
      name: chip_sw_soc_proxy_external_irqs
      desc: '''Verify that external IRQs are forwarded to the PLIC.

               For each of the external IRQs:
               - In DV env, trigger the external IRQ.
               - In DV env, ensure that an internal wakeup is requested.
               - In SW, ensure that the IRQ shows up in the respective *Interrupt Pending* register of the PLIC.
            '''
      stage: V2
      tests: ["chip_sw_soc_proxy_smoketest"]
    }
    {
      name: chip_sw_soc_proxy_external_wakeup_requests
      desc: '''Verify that external wakeup requests are forwarded to Power Manager.

               - In SW, enable the wakeup in Power Manager.
               - In DV env, trigger the wakeup request.
               - In SW, ensure that the wakeup request shows up in Power Manager's `WAKE_STATUS` register.
            '''
      stage: V2
    ...
    }
    {
      ...
      desc: '''Verify that GPI/Os of the SoC into/from OT are correctly connected to multiplexed I/Os.

               For each of the general-purpose outputs of the SoC into OT
               - In SW, configure Pinmux MIO such that the pin is externally observable.
               - In DV env, toggle the pin.
               - In DV env, ensure that the pattern shows up on the configured pin.

               For each of the general-purpose inputs of the SoC from OT
               - In SW, configure Pinmux MIO such that the pin is externally controllable.
               - In DV env, toggle the pin.
               - In DV env, ensure that the pattern shows up on the SoC input.
            '''
      stage: V2
      tests: ["chip_sw_soc_proxy_gpios"]
    }
   ...
   }
   {
      ...
      desc: '''Verify the functionality of the ibex fault dump.
               ...
               - Purposely create an ibex exception during execution through reads to an unmapped
                 address.
   ...
   }
   {
    ...
    ////////////////////////////
    {
      ...
      desc: '''Provide example tests for different testing scenarios / needs.
            ...
            4. Enable external manufacturer hooks in existing tests developed in the open source
    ...
    }
    {
      ...
      desc: '''Walk through the life cycle stages from RAW state and resetting the chip each time.
    ...
    }
    {
      ...
      desc: '''Configure RAW_UNLOCK via LC TAP interface and enable CPU execution.
            ...
            right after transition into test_unlocked0 state. The test also verifies that
    ...
    }
    {
      ...
      Blocks / functionality to run simultaneously in this test:
        ...
        - Simultaneous IO toggling as defined below
          - ideally for digital activity, UART / I2C modules should be activated
...

    {
      ...
      - In the test program, check whether the OTP partition containing the scramble seeds is
        locked. Also check that the unscrambled value programmed into flash isolated partition
