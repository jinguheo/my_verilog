# Spec Document: opentitan/hw/ip/aes/data/aes_sec_cm_testplan.hjson

- Project: `opentitan`
- Original source: `D:\MyWork\verilog\dbs\opentitan\hw\ip\aes\data\aes_sec_cm_testplan.hjson`
- Exported path: `D:\MyWork\verilog\out\spec_documents_20260514_204108\opentitan\hw\ip\aes\data\aes_sec_cm_testplan.hjson`
- Original extension: `.hjson`
- Original bytes: 12723
...
// .../aes/data/aes_testplan.hjson
{
  testpoints: [
    ...
    {
      name: sec_cm_gcm_config_shadow
      desc: "Verify the countermeasure(s) GCM.CONFIG.SHADOW."
      stage: V2S
      tests: ["aes_shadow_reg_errors"]
    }
    {
    ...
    {
      name: sec_cm_gcm_config_sparse
      desc: '''
            Verify the countermeasure(s) GCM.CONFIG.SPARSE.
            Illegally encoded values as well as currently disallowed but legal values are written into the GCM control register via register interface and it is ensured that the values are resolved to the correct legal values.
            Internal wires carrying the corresponding signals are forced to invalid values and it is ensured that the DUT stops processing data.
            '''
      stage: V2S
      tests: ["aes_stress", "aes_config_error", "aes_gcm_save_restore", "aes_core_fi"]
    }
    {
    ...
    {
      name: sec_cm_ghash_fsm_sparse
      desc: '''
            Verify the countermeasure(s) GHASH.FSM.SPARSE.
            It is ensured that upon forcing the state register of the GHASH FSM to an invalid value, the DUT stops processing data, locks up and triggers a fatal alert.
            '''
      stage: V2S
      tests: ["aes_fi"]
    }
    ...
    {
      name: sec_cm_ghash_fsm_local_esc
      desc: '''
            Verify the countermeasure(s) GHASH.FSM.LOCAL_ESC.
            Upon detecting a local alert condition it is ensured that the DUT stops processing data, locks up and triggers a fatal alert.
            '''
      stage: V2S
      tests: ["aes_fi", "aes_ghash_fi"]
    }
    {
