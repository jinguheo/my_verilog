...
- **Programmer's Guide**: There are in general only two software controllable functions in the clock manager.
- **Transactional Clock Hints**: There are in general only two software controllable functions in the clock manager.
- **Peripheral Clock Controls**: To enable a transactional clock, set the corresponding hint in [`CLK_HINTS`](registers.md#clk_hints) to `1`.
...
### Programmer's Guide
_Source: `opentitan/hw/ip_templates/clkmgr/doc/programmers_guide.md`_
...
Note, a `0` does not indicate clock is actu
…
```
...
To enable a transactional clock, set the corresponding hint in [`CLK_HINTS`](registers.md#clk_hints) to `1`.
To disable a transactional clock, set the corresponding hint in [`CLK_HINTS`](registers.md#clk_hints) to `0`.
Note, a `0` does not indicate clock is actually disabled, software can thus check [`CLK_HINTS_STATUS`](registers.md#clk_hints_status) for the actual st
…
```

### Hardware Interfaces
_Source: `opentitan/hw/top_darjeeling/ip_autogen/clkmgr/doc/interfaces.md`_
...
### Programmer's Guide
_Source: `opentitan/hw/top_darjeeling/ip_autogen/clkmgr/doc/programmers_guide.md`_
...
**RTL** (17)
  ...
  - `clkmgr.sv`:L1 — `opentitan\hw\top_darjeeling\ip_autogen\clkmgr\rtl\clkmgr.sv`
  - `clkmgr`:L10 — `opentitan\hw\top_darjeeling\ip_autogen\clkmgr\rtl\clkmgr.sv`
  - `clkmgr_reg_pkg`:L34 — `opentitan\hw\top_englishbreakfast\ip_autogen\clkmgr\rtl\clkmgr_reg_top.sv`
  - `clkmgr_reg_top`:L231 — `opentitan\hw\top_englishbreakfast\ip_autogen\clkmgr\rtl\clkmgr.sv`
**DV** (11)
  ...
  - `clkmgr_test_pkg.sv`:L1 — `opentitan\hw\top_darjeeling\ip_autogen\clkmgr\dv\tests\clkmgr_test_pkg.sv`
**SVA** (22)
...
- `rstmgr` (41 refs; instantiates×37, imports_from×4)
- `rv_core_ibex` (35 refs; imports_from×27, instantiates×8)
- `lowrisc_ibex` (21 refs; instantiates×18, imports_from×3)
- `rv_plic` (18 refs; instantiates×18)
- `pwrmgr` (17 refs; imports_from×11, instantiates×6)
- `pulp_riscv_dbg` (14 refs; instantiates×14)
- `flash_ctrl` (10 refs; instantiates×10)
- `pinmux` (3 refs; instantiates×3)
- `spi_device` (3 refs; instantiates×3)
...
- For code-only queries mentioning `clkmgr`, expand toward spec anchors via this description.
- Spec Excerpts above show primary functional context — prefer these over raw file lists.
- Bridge table maps the exact spec ↔ code correspondences found by Graphify.
- Neighbor components listed above share code-level relationships with `clkmgr`.
